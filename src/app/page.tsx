...
export default function Home() {
  ...
  const { data: chats, mutate: mutateChats } = useSWR<ChatWithMessage[]>(
    'chats',
    fetcher,
    {
      fallbackData: [],
  ...
  const onSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!chatId) {
      const newChat: ChatWithMessage = await ClientHttp.post('chats', { message })
  ...
  return (
    <main className="w-full h-full flex overflow-hidden">
      <aside className="bg-gray-900 w-72 h-screen p-2 flex flex-col">
        <button
          className='flex gap-3 items-center mb-2 p-3 border border-white/20 rounded hover:bg-gray-500/10 text-gray-100'
          onClick={() => { 
            router.push('/');
            setChatId(null);
            setMessageId(null)
          ...
          <div className="mb-6 mx-auto max-w-3xl">
            <form id="form" onSubmit={onSubmit}>
              <div className="flex flex-col py-3 pl-4 relative text-white bg-gray-700 rounded">
                ...
                <button
                  type="submit"
                  className="absolute top-2 text-gray-400 bottom-2.5 rounded hover:text-gray-400 hover:bg-gray-900 md:right-4"
